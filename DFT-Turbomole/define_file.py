...
if __name__ == '__main__':
    ...
    with open(file_name, 'w') as f:
        ...
        f.write('\n')
        f.write(wano_file["Molecular-structure"]["Title"]+'\n')
        f.write('\n')
        f.write('\n')
        f.write('\n')
        ...
        if wano_file["Molecular-structure"]["Dihedral-Scan (SIMONA)"]:
            DH_file = wano_file['Molecular-structure']['Dihedral-File']
            id_list = []
            #read the yalm file
            with open(DH_file, 'r') as input:
                data_loaded = yaml.safe_load(input)
                id_list = data_loaded['id_list']
            print(' dihedral scan frozen torsion ids=', id_list)
        ...
        if wano_file["Molecular-structure"]["Frozen-torsion"]:
            id_list = []
            xx = wano_file['Molecular-structure']['Frozen']
            for var in xx:
                y = var['Atom id']
                id_list.append(y)
            id_list = " ".join([i.strip() for i in id_list])
            print(' frozen torsion ids=', id_list)

            f.write('ired\n')
            f.write('i\n')
            f.write('idef\n')
            f.write('f tors ' + id_list + ' \n')
            f.write('\n')
            f.write('\n')
            f.write('\n')
            f.write('ired\n')
